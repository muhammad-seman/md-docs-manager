...
async function loadFiles() {
    if (!state.repo.owner || !state.repo.name) {
        elements.fileList.innerHTML = '<p class="empty-state">Configure repository settings first</p>';
        return;
    }
    ...
    try {
        ...
        try {
            const contents = await githubAPI(`/repos/${state.repo.owner}/${state.repo.name}/contents/${path}`);
            state.files = contents.filter(file => file.name.endsWith('.md'));
            renderFileList();
        } catch (error) {
            if (error.message.includes('404')) {
                elements.fileList.innerHTML = '<p class="empty-state">No documents found. Upload your first .md file!</p>';
                state.files = [];
            } else {
                throw error;
            }
...
async function saveFileToGitHub(fileName, content, message = 'Update document', sha = null) {
    try {
        showLoading(true);
        ...
        await githubAPI(`/repos/${state.repo.owner}/${state.repo.name}/contents/${path}`, {
            method: 'PUT',
...
async function deleteFileFromGitHub(file) {
    ...
    try {
        showLoading(true);
        await githubAPI(`/repos/${state.repo.owner}/${state.repo.name}/contents/${file.path}`, {
            method: 'DELETE',
            body: JSON.stringify({
...
function updateUI() {
    if (state.user) {
        ...
        elements.fileList.innerHTML = '<p class="empty-state">Login untuk melihat dokumen</p>';
    }
}

function renderFileList(files = state.files) {
    if (files.length === 0) {
        elements.fileList.innerHTML = '<p class="empty-state">No documents found</p>';
        return;
    }
